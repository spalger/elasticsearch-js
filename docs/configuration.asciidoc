...
+
`components` is a map of the internal classes for this version of the elasticsearch client. The values on this object are listed https://github.com/elastic/elasticsearch-js/blob/master/src/elasticsearch-js/lib/client.js#L80[here].
...
Default:::
see https://github.com/elasticsearch/elasticsearch-js/blob/master/src/elasticsearch-js/lib/nodes_to_host.js[nodes_to_host.js]
...
+
The function is called with two arguments, first an `HttpConnector`[http://github.com/spalger/elasticsearch-js/blob/master/src/elasticsearch-js/lib/connectors/http.js] object and the second the config object initially passed when creating the client.

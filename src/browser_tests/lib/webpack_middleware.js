...
export async function withWebpackMiddleware(block) {
  ...
  try {
    ...
    compiler.plugin('watch-run', (_, done) => {
      runCount += 1;
      if (runCount === 1) {
        log.info('webpack compiling bundles...');
      } else {
        log.debug('webpack compiling bundles due to changes in files...');
      }
      done();
    });
    compiler.plugin('done', stats => {
      if (!stats.hasErrors()) {
        ...
        log.debug('webpack compilation successful in', durationSec.toFixed(2), 'seconds');
      }
    });
